...
const UIModule = (() => {
  function showToast(message, type = 'success') {
    ...
    toast.className = `toast ${type === 'success' ? 'bg-green-600' : type === 'info' ? 'bg-blue-600' : 'bg-red-600'}`;
  ...
  function switchTab(tabName) {
    document.querySelectorAll('.nav-tab').forEach(tab => {
      tab.classList.remove('bg-gray-400','text-black');
      tab.classList.add('text-gray-700','hover:bg-gray-600');
    });
    const activeTab = document.querySelector(`[data-tab="${tabName}"]`);
    if (activeTab) {
      activeTab.classList.remove('text-gray-700','hover:bg-gray-900');
      activeTab.classList.add('bg-gray-400','text-black');
    }
    ...
    else if (tabName === 'settings') { SettingsModule.loadPricingForm(); SettingsModule.loadOperatingHoursForm(); EmailModule.loadEmailConfigForm(); SettingsModule.loadGcashQRSection(); }
