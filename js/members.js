...
const MembersModule = (() => {
  ...
  // ── GCASH PAYMENT MODAL ───────────────────────
  let _gcashPending = null;

  function showGcashModal(amount, memberName, onConfirmCallback) {
    const qr = SettingsModule.getGcashQR();
    document.getElementById('gcashPayAmount').textContent = '₱' + amount.toLocaleString();
    document.getElementById('gcashPayMemberName').textContent = memberName;

    const img    = document.getElementById('gcashQRImg');
    const notSet = document.getElementById('gcashQRNotSet');
    if (qr) {
      img.src = qr;
    ...
    _gcashPending = onConfirmCallback;
    document.getElementById('gcashPayModal').classList.add('active');
  }

  function confirmGcashPayment() {
    document.getElementById('gcashPayModal').classList.remove('active');
    if (_gcashPending) { _gcashPending(); _gcashPending = null; }
  }

  function cancelGcashPayment() {
    _gcashPending = null;
    document.getElementById('gcashPayModal').classList.remove('active');
    UIModule.showToast('GCash payment cancelled.', 'info');
  }

  // ── ADD MEMBER ────────────────────────────────
  async function addMember(memberData) {
    if (memberData.paymentMethod === 'GCash') {
      const prices = Config.getMembershipPrices();
      const amount = prices[memberData.membershipType] || 0;
      showGcashModal(amount, memberData.name, () => _processAddMember(memberData));
      return { gcashPending: true };
    }
    return await _processAddMember(memberData);
  }

  async function _processAddMember(memberData) {
  ...
  function renderPaymentSummaryTable() {
    const tbody = document.getElementById('paymentSummaryBody');
    if (!tbody) return;

    const allMembers  = StorageModule.getAllMembers();
    const allPayments = StorageModule.getAllPayments();
    const today       = new Date().toISOString().split('T')[0];

    if (allMembers.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" class="px-4 py-6 text-center text-gray-400">No members yet</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = allMembers
      ...
      .map(member => {
        ...
        const feeLabel = member.membershipType.includes('Monthly') ? 'monthly' : 'per session';
        const isPaid   = memberPayments.length > 0 && member.status === 'Active';
        ...
        const statusHtml = isPaid
          ? '<span class="font-semibold" style="color:#22c55e;">Paid</span>'
          : '<span class="font-semibold" style="color:#f59e0b;">Pending</span>';

        const expiryDisplay = member.expiryDate
          ? `<span class="${daysLeft < 0 ? 'text-red-500 font-bold' : daysLeft <= 7 ? 'text-orange-500 font-semibold' : 'text-gray-700'}">${member.expiryDate}</span>`
          : '—';

        return `<tr class="border-b border-gray-100 hover:bg-gray-50 transition">
          <td class="px-4 py-3 text-sm font-medium text-gray-800">${member.name}</td>
          <td class="px-4 py-3 text-sm text-gray-600 text-center">${feeLabel}</td>
          <td class="px-4 py-3 text-sm text-center">${statusHtml}</td>
          <td class="px-4 py-3 text-sm text-center">${expiryDisplay}</td>
        </tr>`;
      }).join('');
  }
  ...
  function filterMembers(query) {
    ...
    } else if (countEl) { countEl.classList.add('hidden'); }
      ...
      <div class="p-4 border border-gray-200 rounded-lg hover:shadow-md transition">
        <div class="flex justify-between items-start">
          <div class="flex items-center space-x-4">
            ...
            <div class="flex-1">
              <div class="flex items-center space-x-2">
                ...
                ${member.profilePhoto
                  ? '<span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">🔒 Photo Set</span>'
                  : '<span class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full font-medium">⚠️ No Photo</span>'}
              </div>
          ...
          <div class="flex flex-col items-end space-y-2">
            <span class="px-3 py-1 rounded-full text-sm font-medium ${member.status === 'Active' ? 'bg-emerald-500 text-black' : 'bg-red-100 text-red-700'}">${member.status}</span>
  ...
  function previewPhoto(event) {
    ...
    _resizeImage(file, (dataUrl) => {
      ...
      placeholder.classList.add('hidden'); clearBtn.classList.remove('hidden');
    });
  }
  ...
  function closeRenewModal() { renewMemberId = null; document.getElementById('renewModal').classList.remove('active'); }
  ...
  async function confirmRenewal() {
    ...
    if (payment === 'GCash') {
      const prices = Config.getMembershipPrices();
      showGcashModal(prices[type] || 0, member.name, () => _processRenewal(member, type, start, payment));
      return;
    }

    await _processRenewal(member, type, start, payment);
  }

  async function _processRenewal(member, type, start, payment) {
    ...
    StorageModule.addPayment({
      id: Date.now().toString(), memberId: member.id, memberName: member.name,
      amount: prices[type] || 0, method: payment, date: new Date().toISOString(), type: 'Renewal'
    });
  ...
  return {
    addMember, getActiveMembers, getExpiringSoonMembers,
    renderMembersList, filterMembers, renderPaymentSummaryTable,
    confirmDeleteMember, cancelDeleteMember, executeDeleteMember,
    previewPhoto, clearPhoto, getPhotoDataUrl, triggerPhotoUpdate, updateMemberPhoto,
    openRenewModal, closeRenewModal, updateRenewPreview, confirmRenewal,
    confirmGcashPayment, cancelGcashPayment
  };
})();
