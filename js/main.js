...
const App = (() => {
  ...
  function handleLogin(e) {
    ...
    if (result.success) {
      ...
      if (attempts >= MAX_ATTEMPTS) {
        ...
        warningEl.innerHTML = `<div class="flex items-start gap-2"><span class="text-xl">🚫</span><div><p class="font-bold">Too many failed attempts!</p><p class="text-sm mt-1">Please contact your <strong>admin</strong> to verify your password or reset it in the Staff Account Management.</p></div></div>`;
      } else if (attempts >= 3) {
        warningEl.classList.add('bg-orange-50','border-orange-400','text-orange-800');
        warningEl.innerHTML = `<div class="flex items-start gap-2"><span class="text-xl">⚠️</span><div><p class="font-bold">Incorrect username or password</p><p class="text-sm mt-1">Only <strong>${remaining} attempt${remaining !== 1 ? 's' : ''} remaining</strong>. Please double-check your credentials or use Forgot Password.</p></div></div>`;
      } else {
        warningEl.classList.add('bg-yellow-50','border-yellow-400','text-yellow-800');
        warningEl.innerHTML = `<div class="flex items-start gap-2"><span class="text-xl">⚠️</span><div><p class="font-bold">Incorrect username or password</p><p class="text-sm mt-1">${remaining} attempt${remaining !== 1 ? 's' : ''} remaining before you are advised to contact admin.</p></div></div>`;
      }
    }
  }
  ...
  async function handleAddMember(e) {
    ...
    if (result.success) {
      ...
      setInitialDate();
    } else if (result.gcashPending) {
      // GCash modal is showing — reset form in background
      document.getElementById('addMemberForm').reset();
      MembersModule.clearPhoto();
      setInitialDate();
    }
  ...
  function handleEmailConfigUpdate(e) {
    ...
    EmailModule.saveEmailConfig({
      ...
      otpTemplateId: otpTpl ? otpTpl.value : ''
    });
  ...
  function handlePricingUpdate(e) {
    e.preventDefault();
    const result = SettingsModule.updatePricing({
      dayPassStudent:  document.getElementById('dayPassStudentPrice').value,
      dayPassRegular:  document.getElementById('dayPassRegularPrice').value,
      monthlyStudent:  document.getElementById('monthlyStudentPrice').value,
      monthlyRegular:  document.getElementById('monthlyRegularPrice').value
    });
    UIModule.showToast(result.message, 'success');
  }
  ...
  function handleOperatingHoursUpdate(e) {
    ...
    ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].forEach(day => {
      if (document.getElementById(`day${day}`).checked) days.push(day);
    });
    const result = SettingsModule.updateOperatingHours({
  ...
  return { init, confirmLogout, cancelLogout };
})();
